...
class DHGLayer(nn.Module):
    ...
    def nearest_select(self, ids, feats):
        dis = cos_dis(feats)
        # self.kn
        _, idx = torch.topk(dis, 30, dim=1)
        idx = idx[ids]
        edge_list = idx.numpy().tolist()
        c = Attention(dim_in=feats.size(1))
        knn_edges = c.forward(feats, edge_list, self.kn)
        idx = torch.LongTensor(knn_edges)
        # print(edge_list)
        N = len(idx)
        d = feats.size(1)
